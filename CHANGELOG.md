...
### Changed
- Endless runs now return to camp through explicit checkpoints: after each Boss reward chain, players choose to continue or save the current run and visit camp; endless death and confirmed pause-menu surrender clear the run and return to camp, while selecting a slot with an active run resumes battle directly. Pending Boss rewards no longer overwrite the last safe run snapshot, and Developer Mode no longer clears an official run. Fixed endless-mode context restoration that could leave Ruan stones unconfigured, and added visible thunder/flame links plus thunder/flame/poison hit bursts.
- Added the first endless-mode meta loop: normal enemies have a 1% bone drop, elites drop 1, small Bosses 3, and Bosses 8; collected bones persist immediately while uncollected pickups survive continue-save restoration. Ruan Dog now opens a five-card collection panel for permanent, uncapped thunder/frost/poison/flame/fury stone upgrades with linear `5 + 3L` costs and one shared active stone. Direct basic attacks carry authored event IDs so chain/split trigger once per event, target statuses deduplicate per enemy, and secondary stone damage cannot recursively trigger combat talents. HUD, character panel, legacy endless-profile migration, and non-persistent developer controls are synchronized.
