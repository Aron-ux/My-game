...
class FreeingDamageOwner:
	...
	func _deal_damage_to_enemy(enemy: Node, _damage_amount: float, _source_role_id: String, _vulnerability_bonus: float = 0.0, _vulnerability_duration: float = 2.0, _slow_multiplier: float = 1.0, _slow_duration: float = 0.0, _source_position: Variant = null, _suppress_status_visual: bool = false, _kill_energy_bonus: float = 0.0) -> bool:
		damage_calls += 1
