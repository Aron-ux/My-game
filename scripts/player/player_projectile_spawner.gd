...
static func spawn_batched_directional_bullet(owner, direction: Vector2, damage_amount: float, color: Color, role_id: String = "", origin: Variant = null, config: Dictionary = {}) -> bool:
	...
	return bool(batch.add_projectile({
		...
		"gunner_basic_split_enabled": bool(config.get("gunner_basic_split_enabled", false)),
		"gunner_basic_split_count": int(config.get("gunner_basic_split_count", 0)),
		"gunner_basic_split_arc_degrees": float(config.get("gunner_basic_split_arc_degrees", 0.0)),
		"gunner_basic_split_damage": float(config.get("gunner_basic_split_damage", 0.0)),
		"gunner_basic_split_lifetime_scale": float(config.get("gunner_basic_split_lifetime_scale", 1.0)),
		"gunner_basic_split_speed_scale": float(config.get("gunner_basic_split_speed_scale", 1.0)),
		"gunner_basic_split_visual_scale": float(config.get("gunner_basic_split_visual_scale", 1.0)),
		"wave_elapsed": 0.0,
