...
func _perform_combo_segment(owner, base_direction: Vector2, combo_scale: float, allow_trick_variants: bool = true, allow_followthrough: bool = true) -> void:
	...
	if allow_trick_variants:
		total_hits += _apply_trick_variants(owner, base_direction)
...
func _apply_trick_variants(owner, base_direction: Vector2) -> int:
	var total_hits := 0
	var index := 1
	for scale in _get_skill_effect_scales(owner, "quantity_skill_count"):
		...
		total_hits += _perform_attack_variant(owner, direction, float(scale), false, false, false)
		index += 1
	return total_hits
