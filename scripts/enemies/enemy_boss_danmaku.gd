	...
	for index in range(count):
		...
		match pattern:
			...
			1:
				...
				speed += 16.0
				angular = side * 0.20
				hue = 0.63 if side < 0.0 else 0.77
			2:
				...
				hue = 0.67 + wave * 0.02
			3:
				# Three staggered fracture fronts. The aggregate silhouette
				# is a broken shock ring, matching the corrupted stone.
				angle += wave * 0.06 * spin + side * 0.025
