...
const ROLE_INTRO_CONTENT := {
	...
	"gunner": {
		"name": "枪手",
		"trait": "特性机制：枪手拥有半径 115 的【猎杀】安全区，圈内敌人承受的枪手伤害只有 40%；圈外敌人则会按与枪手的距离继续提高承伤。枪手基础闪避率为 15%。",
		"buff": "Buff 机制：【瞬杀】会在枪手持续不受伤时每 2 秒叠加 1 层，最多 10 层；每层提供 3% 伤害、3% 移速和 1.5% 闪避率。枪手一旦受伤会立刻清空层数，并进入 15 秒重置冷却。枪手默认减伤值为 0。",
...
const ROLE_INTRO_CONTENT_OVERRIDE := {
	...
	"gunner": {
		"name": "枪手",
		"trait": "特性：[猎杀]会在枪手周围生成半径115的安全区，圈内敌人承受的枪手伤害只有40%，圈外敌人则会随距离继续提高承伤。枪手基础闪避率15％；最终闪避率由基础闪避率、永久闪避值和临时闪避强化共同决定。\n\n登场技：枪手登场时释放[枪火典礼]，连续打出3波8方向高伤子弹，每发造成200％伤害。",
		"buff": "[瞬杀]：枪手每2秒没有受到伤害获得一层[瞬杀]，每层提供3％伤害、3％移速和1.5%闪避率，最多10层。受到伤害后全部层数移除并进入15秒冷却时间。",
		"entry": ""
	},
	"mage": {
		"name": "法师",
