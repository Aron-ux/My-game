...
const TEXT_SUBTITLE := "选择存档，进入营地配置 N 层挑战"
...
func _on_slot_pressed(slot_id: int, has_profile: bool, _has_run: bool) -> void:
	if not has_profile:
		SAVE_MANAGER.create_or_load_endless_profile(slot_id)
